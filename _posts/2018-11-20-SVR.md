...
$$ {L}_{D} = \frac {1} {2} \lVert \sum _{i,j=1}^{n} (\alpha_{i}^{*}-\alpha_{i})(\alpha_{j}^{*}-\alpha_{j})K(\mathbf{x}_{i},\mathbf{x}_{j}) - \epsilon \sum _{i,j=1}^{n} (\alpha_{i}+\alpha_{i}^{*}) + \sum _{i,j=1}^{n} y_{i}(\alpha_{i}^{*}-\alpha_{i}) $$  
